...
namespace UniGenVR {
    ...
    // the camera.
    public class CameraBoundElement : MonoBehaviour {
        [SerializeField] private bool m_LookatCamera = true;    // Whether the UI element should rotate to face the camera.
        [SerializeField] private Transform m_Element;         // The transform of the UI to be affected.
        ...
        private void Start() {
            ...
            m_DistanceFromCamera = Vector3.Distance(m_Element.position, m_Camera.position);
        }
        ...
        private void Update() {
            ...
            if (m_LookatCamera)
                m_Element.rotation = Quaternion.LookRotation(m_Element.position - m_Camera.position);
            ...
            if (m_RotateWithCamera) {
                ...
                targetPosition = Vector3.Lerp(m_Element.position, targetPosition, m_FollowSpeed * Time.deltaTime);

                // Since the UI is only following on the XZ plane, negate any y movement.
                targetPosition.y = m_Element.position.y;

                // Set the UI's position to the calculated target position.
                m_Element.position = targetPosition;
            }
        }
    }
}